"""
This script includes functions to calibrate the Tynitag and geoprecision thermistors at 0 degrees Celsius.
...
def calculate_chain_zero_degree_offsets(df, threshold=0.1, window=10):
    """
    Calculate zero degree offsets for all thermistor columns in a chain DataFrame.

    Parameters:
    - df: pd.DataFrame, the calibration data (columns must be simplified, e.g. '#1', '#2', ...)
    - threshold: float, stability threshold for offset calculation
    - window: int, rolling window size

    Returns:
    - dict: {thermistor_column: zero_degree_offset}
    """
    offsets = {}
    for col in df.columns:
        if col.startswith("#"):
            offset, _ = calculate_zero_degree_offset(df[col], threshold=threshold, window=window)
            offsets[col] = offset
    return offsets

def apply_chain_offsets(df, offsets):
    """
    Adds the offset from the offsets dict to each matching column in df.
    df: pandas DataFrame with thermistor data
    offsets: dict with column names as keys and offset values as values
    """
